...
## 스크린샷 모드 (README 데모 이미지)

`./screenshot.sh`가 mock 데이터로 팝오버를 캡처해 `screenshots/popover.png`를 만든다(계정·네트워크 무관, 항상 동일).

- `--screenshot` → `AppModel.isScreenshotMode`(폴링·네트워크 차단) + `MockData` 주입. App body의 `MenuBarExtra`는
  `SceneBuilder`가 if/else를 못 받아 분기 불가라, 별도 윈도우는 `AppDelegate`가 띄운다(borderless·11pt 모서리·key 윈도우).
- **글래스 백드롭 재현**: 실제 메뉴바 팝오버의 시스템 글래스를 `NSVisualEffectView`(`.menu`, behindWindow)로 흉내 낸다(`PopoverBackdrop`).
  behindWindow는 윈도우 *뒤*를 블러하므로, 뒤에 고정 그라데이션 배경 윈도우를 깔아 캡처마다 일관되게 한다.
  - **실제 앱엔 적용하지 않는다**: `MenuBarExtra`의 시스템 백드롭과 중첩되면 오히려 더 불투명해진다(직접 확인). 실제 앱은 시스템 백드롭을 그대로 쓴다.
- **`AvatarView`(.task)**: `AsyncImage`가 `NSHostingController` 컨텍스트에서 로드 task를 트리거하지 않아 아바타가 회색으로 남는다.
  `.task`로 직접 `URLSession` 다운로드해 표시한다. 스크린샷뿐 아니라 실제 앱 아바타에도 적용된다.
...
7. **스크린샷 모드** → README 데모용. mock 데이터 + 별도 윈도우(`AppDelegate`) + 글래스 백드롭 재현(실제 앱엔 미적용). `AvatarView(.task)`로 `AsyncImage` 로드 이슈 해결.
